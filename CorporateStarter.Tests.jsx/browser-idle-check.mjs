...
const context=vm.createContext({
 ...
 fetch:async(url,opts)=>{
  calls.push({url,opts});
  if(url.endsWith('/login'))return {status:200,json:async()=>({accessToken:'private-token',expiresAtUtc:new Date(Date.now()+300000).toISOString(),user:{id:uid}})};
  if(url.endsWith('/logout'))return {status:204};
  return {status:nextStatus,json:async()=>({userId:uid,remainingMilliseconds:1800000})};
 }
});
...
nextStatus=200;await api.logout();const count=calls.length;now=128000;await timer();assert.equal(calls.length,count,'No reports after logout');
